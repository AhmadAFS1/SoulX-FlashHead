...
def test_four_step_resolves_to_the_recorded_distilled_levels():
    ...
    resolved = resolved_timestep_schedule(4, shift=5.0)
...
def test_every_level_is_a_member_of_the_distilled_set(steps):
    ...
    for level in resolved_timestep_schedule(steps, shift=5.0):
...
def test_regression_the_old_linspace_defect_is_gone():
    ...
    resolved = [round(t, 4) for t in resolved_timestep_schedule(3, shift=5.0)]
...
def test_untabulated_step_count_raises_rather_than_interpolating():
    with pytest.raises(ValueError, match="No tabulated schedule"):
        raw_timestep_schedule(7)


def test_raw_schedule_appends_exactly_one_terminal_zero():
    for steps in (1, 2, 3, 4):
        raw = raw_timestep_schedule(steps)
        assert raw[-1] == 0.0
