"""Recreate one SoulX FlashHead variant for the 1.50x PRO-vs-LITE comparison.

The original lite/video.mp4 and pro/video.mp4 for this benchmark are no longer
present in the working tree (only their results.json metrics survived). This
script reruns fresh GPU inference using the exact surviving reference.png and
audio.wav from the parent benchmark directory (sha256-verified against the
values recorded in ../README.md) and the exact parameters documented there.
It is a fresh recreation, not a byte-identical replay of the original run.
"""
import argparse
import hashlib
import json
...
REFERENCE = PARENT / "reference-150x.png"
...
def main():
    ...
    result = {
        ...
        "execution": "fresh local GPU inference (recreation; original video bytes were missing)",
