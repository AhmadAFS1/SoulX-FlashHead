...
def main():
    output = ROOT / "soulx-LITE-vs-PRO-indian-man-square-closeup-512x512-seed42.mp4"
    command = [
    ...
    for variant in ("lite", "pro"):
        ...
        assert int(video["nb_read_frames"]) in (229, 249)
        assert (int(video["width"]), int(video["height"])) == (512, 512)
    ...
    assert int(comparison_video["nb_read_frames"]) in (229, 249)
    assert (int(comparison_video["width"]), int(comparison_video["height"])) == (1024, 512)
