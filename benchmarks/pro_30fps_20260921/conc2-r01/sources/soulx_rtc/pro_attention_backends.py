...
def kernel_respects_current_stream(kernel: Callable[..., torch.Tensor], kwargs: dict[str, Any],
                                   device: torch.device | str = "cuda", trials: int = 3) -> bool:
    """Empirical check that ``kernel`` launches on ``torch.cuda.current_stream()``.

    The SM89 SageAttention 2.2.0 build used here launches its attention and fused
    quantisation kernels with ``<<<grid, block, smem>>>`` -- no stream argument, i.e. the
    legacy default stream -- while everything else in the pipeline runs on the current
    stream. That is invisible when the whole pipeline runs on the default stream (every
    single-stream measurement so far) and fatal the moment a session runs on its own
    stream: the kernel reads q/k/v before the producing kernels have written them. The
    first two-session run produced non-finite pixels in window 0 for exactly this reason.

    Method: build q/k/v at the end of a ~100 ms dependent matmul chain on a fresh
    non-default stream and call the kernel there. A kernel on the wrong stream starts
    long before the chain finishes and its output differs from (or is not even finite
    against) the fully synchronised reference computed from identical inputs.
    """
    device = torch.device(device)
    torch.cuda.synchronize(device)
    weight = torch.randn(4096, 4096, device=device, dtype=torch.float16) * 0.02
    batch, tokens, heads, head_dim = 1, 4032, 12, 128

    def chain(trial: int) -> torch.Tensor:
        generator = torch.Generator(device=device).manual_seed(trial)
        x = torch.randn(4608, 4096, device=device, dtype=torch.float16, generator=generator)
        for _ in range(60):
            # layer_norm keeps the chain bounded; an unnormalised silu(x @ W) chain
            # overflows fp16 within ~40 steps and turns the probe into a NaN test.
            x = torch.nn.functional.layer_norm(torch.nn.functional.silu(x @ weight), (4096,))
        return x

    def inputs(x: torch.Tensor):
        flat = x.flatten()[: 3 * batch * tokens * heads * head_dim].to(torch.bfloat16)
        q, k, v = flat.view(3, batch, tokens, heads, head_dim).unbind(0)
    ...
    for trial in range(trials):
        ...
        with torch.cuda.stream(stream):
            out = kernel(*inputs(chain(trial)), **kwargs)
        stream.synchronize()
        torch.cuda.synchronize(device)
        x = chain(trial)
        torch.cuda.synchronize(device)
        ref = kernel(*inputs(x), **kwargs)
        torch.cuda.synchronize(device)
        if not bool(torch.isfinite(out).all()) or bool((out.float() != ref.float()).any()):
            safe = False
            break
    del weight
    torch.cuda.synchronize(device)
...
def _call_on_default_stream(kernel: Callable[..., torch.Tensor], q, k, v, kwargs: dict[str, Any]):
    """Run a kernel that ignores the current stream on the default stream, fenced.

    The default stream waits for everything the caller's stream has issued (q/k/v are
    ready), the kernel and its Python-side quantisation run on the default stream, and
    the caller's stream then waits for them. ``record_stream`` keeps the caching
    allocator from handing the output block -- allocated on the default stream -- to
    another default-stream user while the caller is still reading it. When the caller
    already is the default stream this is a plain call. Dynamo never sees any of it: the
    pybind kernel graph-breaks the compiled DiT here anyway, and stream ops would only add
    more breaks.
    """
    current = torch.cuda.current_stream(q.device)
    default = torch.cuda.default_stream(q.device)
    if current == default:
        return kernel(q, k, v, **kwargs)
    default.wait_stream(current)
    with torch.cuda.stream(default):
        output = kernel(q, k, v, **kwargs)
...
class SageAttention2Backend(ExplicitAttentionBackend):
    ...
    def __init__(
        ...
        *,
        ...
        # Decided empirically by prepare(); None means the probe has not run and every
        # call is fenced through the default stream (the safe assumption).
        self.stream_safe: bool | None = None
    ...
    def manifest(self) -> dict[str, Any]:
        return {
            **super().manifest(),
            ...
            "stream_safe": self.stream_safe,
            "stream_fence": (
                "none (kernel launches on the current stream)" if self.stream_safe
                else "default-stream fence around every call (kernel ignores the current stream)"
            ),
            "fallback": False,
        }
    ...
    def prepare(self, device) -> dict[str, Any]:
        """Load the kernel and probe its stream behaviour OUTSIDE any compiled region."""
        kernel = self._load_kernel()
        ...
        if os.environ.get("SOULX_SAGE_FORCE_FENCE") == "1":
            self.stream_safe = False
        else:
            self.stream_safe = kernel_respects_current_stream(kernel, kwargs, device)
        return {"stream_safe": self.stream_safe}
    ...
    def __call__(self, q, k, v, num_heads, *, owner):
        ...
        try:
            ...
            if self.stream_safe:
                ...
                output = _call_on_default_stream(kernel, q_heads, k_heads, v_heads, kwargs)
...
def install_self_attention_backend(
    ...
    *,
    ...
    if prepare is not None:
        # Must run here, eagerly: the first attention call happens inside the compiled
        # DiT's trace, and the stream probe must never be traced by Dynamo.
        prepare(next(model.parameters()).device)
    return installed
