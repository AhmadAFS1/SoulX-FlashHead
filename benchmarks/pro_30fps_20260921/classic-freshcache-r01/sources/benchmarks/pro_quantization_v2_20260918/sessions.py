...
class Session:
    def __init__(self, key: str, seed: int, pipeline) -> None:
        self.key = str(key)
        self.seed = int(seed)
        # SOULX_SESSIONS_DEFAULT_STREAM=1 puts every session on the default stream
        # (bisection aid: isolates stream effects from the rest of the scheduler path).
        import os

        self.stream = (
            torch.cuda.default_stream(pipeline.device)
            if os.environ.get("SOULX_SESSIONS_DEFAULT_STREAM") == "1"
