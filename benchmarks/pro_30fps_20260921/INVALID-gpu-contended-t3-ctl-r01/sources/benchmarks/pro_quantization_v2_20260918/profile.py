...
class ProfileController:
    ...
    def finalize(self, *, status: str, extra: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        result: dict[str, Any] = {
            ...
            "decoder_execution": (
                "eager PyTorch remainder; policy-selected TensorRT engines preserved"
                if getattr(self, "eager_diagnostic", False) else (
                    "eager module inventory" if self.mode == "inventory" else "policy-selected backend preserved"
                )
            ),
        }
        if self.profiler is not None:
