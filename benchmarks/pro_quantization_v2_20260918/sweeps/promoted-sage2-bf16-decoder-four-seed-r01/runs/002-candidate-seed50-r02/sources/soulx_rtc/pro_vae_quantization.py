...
def load_decoder_plan(path: str | Path, *, require_engines: bool = True) -> DecoderPlan:
    ...
    if raw["schema_version"] not in (1, DECODER_PLAN_SCHEMA_VERSION):
    ...
    for target in targets:
        records = engines.get(target.path)
        if records is None:
            continue
        if raw["schema_version"] == 1:
            if len(target.observed_shapes) != 1:
                raise DecoderPlanError("Legacy engines cannot cover multiple observed shapes; rebuild with schema 2")
            records = {shape_key(target.observed_shapes[0]): records}
        if not isinstance(records, Mapping) or set(records) != {shape_key(s) for s in target.observed_shapes}:
            raise DecoderPlanError(f"Engine shape coverage mismatch for {target.path}")
        for key, engine in records.items():
            if not isinstance(engine, Mapping) or set(engine) != {"path", "sha256", "precision"}:
                raise DecoderPlanError(f"Engine record for {target.path}/{key} must contain path, sha256, and precision")
            if engine["precision"] != raw["scheme"].split("_", 1)[0]:
                raise DecoderPlanError(f"Engine precision mismatch for {target.path}/{key}")
        normalized[target.path] = dict(records)
    engines = normalized
    if require_engines and raw["scheme"] != "bf16" and set(engines) != seen:
        raise DecoderPlanError("Quantized decoder plans require one explicit engine record per target and observed shape")
...
class MultiShapeEngine:
    """Dispatch prepared inputs without touching causal cache state."""
    def __init__(self, engines):
        self.engines = dict(engines)

    def __call__(self, value):
        key = shape_key(value.shape)
        if key not in self.engines:
            raise DecoderBackendError(f"No verified decoder engine for prepared shape {key}")
        return self.engines[key](value)
...
class EngineConvolutionBackend(DecoderComputeBackend):
    ...
    def __call__(self, module: nn.Module, prepared_input: torch.Tensor) -> torch.Tensor:
        ...
        spatial = tuple((size + 2 * pad - dilation * (kernel - 1) - 1) // stride + 1
                        for size, pad, dilation, kernel, stride in zip(
                            prepared_input.shape[2:], module.padding, module.dilation,
                            module.kernel_size, module.stride))
        expected = (prepared_input.shape[0], module.out_channels, *spatial)
        if tuple(output.shape) != expected or output.device != prepared_input.device:
            raise DecoderBackendError(f"{self.name} changed output shape/device; expected {expected}")
...
def decoder_module_for_path(vae: Any, path: str) -> nn.Module:
    """Resolve canonical ``model.decoder.*`` paths for Module and WanVAE owners."""
    owners: list[tuple[Any, tuple[str, ...]]] = []
    if hasattr(vae, "get_submodule"):
        owners.append((vae, (path, path.removeprefix("model."), f"model.{path}")))
    model = getattr(vae, "model", None)
    if isinstance(model, nn.Module):
        owners.append((model, (path.removeprefix("model."), path, f"model.{path.removeprefix('model.')}")))
    for owner, candidates in owners:
        for candidate in dict.fromkeys(candidates):
            try:
                return owner.get_submodule(candidate)
