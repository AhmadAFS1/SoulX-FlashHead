...
def _captured_prepared_input(captures: Path, target_path: str, shape: tuple[int, ...]) -> torch.Tensor:
    ...
    raise ValueError(f"No captured prepared CausalConv3d input is available for {target_path}")
...
def _onnx_model(module, input_tensor: torch.Tensor, precision: str, output: Path, calibration: dict) -> dict[str, Any]:
    try:
        import onnx
    ...
    try:
        ...
        for target in plan.targets:
            ...
            for shape in target.observed_shapes:
                ...
                quantization = _onnx_model(module, input_tensor, args.precision, onnx_path, dict(target.calibration))
