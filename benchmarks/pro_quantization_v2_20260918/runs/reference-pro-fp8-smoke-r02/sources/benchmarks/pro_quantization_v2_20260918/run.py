...
from soulx_rtc.pro_attention_backends import install_self_attention_backend, make_attention_backend
...
def _audio_slices(fixture: dict[str, Any], frames: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    samples_per_chunk = 28 * fixture["sample_rate"] // fixture["fps"]
    if samples_per_chunk * fixture["fps"] != 28 * fixture["sample_rate"]:
...
def _install_decoder_policy(pipeline, policy: dict[str, Any], *, capture: bool = False) -> dict[str, Any]:
    ...
    plan_path = resolve_path(decoder["plan"])
    plan = load_decoder_plan(plan_path)
    if plan.scheme != decoder["scheme"]:
        raise ValueError(f"Decoder policy scheme {decoder['scheme']} does not match plan {plan.scheme}")
    from benchmarks.pro_quantization_v2_20260918.build_decoder_engine import TensorRTPreparedConv

    expected_precision = decoder["scheme"].split("_", 1)[0]
    backends = {}
    for target in plan.targets:
        artifact = plan.engines[target.path]
        engine_path = resolve_path(artifact["path"])
        if sha256(engine_path) != artifact["sha256"]:
            raise ValueError(f"Decoder engine hash mismatch for {target.path}")
        if artifact["precision"] != expected_precision:
            raise ValueError(f"Decoder engine precision mismatch for {target.path}")
        runtime = TensorRTPreparedConv(engine_path)
        if runtime.metadata.get("quantization_dispatch_verified") is not True:
            raise RuntimeError(f"Decoder engine lacks explicit {expected_precision} dispatch evidence: {target.path}")
        backends[target.path] = EngineConvolutionBackend(
            runtime,
            name="tensorrt_explicit_qdq",
            precision=expected_precision,
            manifest_data=runtime.metadata,
        )
    return install_decoder_adapters(pipeline.vae, plan, backends)
    ...
    with (output / "stdout.log").open("w", encoding="utf-8") as stdout, (output / "stderr.log").open("w", encoding="utf-8") as stderr:
        ...
        try:
            ...
            pipeline.prepare_params(
                fixture["reference"]["path"], (576, 320), args.frames, 5, 4,
            ...
            if policy["prepared_conditioning"]:
                result["prepared_conditioning"] = _install_prepared_generation(
                    pipeline, compile_dit=policy["compile"]["dit"] and capture_collector is None
                )
