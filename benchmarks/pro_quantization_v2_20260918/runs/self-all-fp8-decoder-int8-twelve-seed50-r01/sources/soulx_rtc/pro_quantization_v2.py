...
SUPPORTED_ATTENTION_BACKENDS = frozenset(("flash2", "sage2"))
SUPPORTED_DECODER_SCHEMES = frozenset((BF16_SCHEME, "int8_conservative", "fp8_conservative"))
SUPPORTED_DECODER_BACKENDS = frozenset(("torch_compile", "pytorch", "adapter", "adapter_compile"))
...
def validate_policy(policy: Mapping[str, Any]) -> None:
    ...
    if decoder["scheme"] == BF16_SCHEME and decoder.get("plan") is not None:
        raise PolicyValidationError("A BF16 decoder policy cannot include a quantized decoder plan")
    if decoder["scheme"] != BF16_SCHEME and decoder.get("backend") not in ("adapter", "adapter_compile"):
