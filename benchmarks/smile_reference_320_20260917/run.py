"""Matched neutral versus teeth-visible smiling reference at 320x576."""
import argparse
import hashlib
import json
import subprocess
...
def main():
    ...
    parser.add_argument("--detail-width", type=int, default=320, choices=[320])
    ...
    result = {
        ...
        "audio": "benchmarks/comparison-10s.wav",
        "audio_sha256": hashlib.sha256(Path("benchmarks/comparison-10s.wav").read_bytes()).hexdigest(),
        "source": str(SOURCE), "source_dimensions": [WIDTH, HEIGHT],
        "effective_source_dimensions": [args.detail_width, detail_height],
        "reference_preparation": (
            "Neutral: exact native original. Smile: imagegen edit resized once to 320x576 with Lanczos. "
            "Both use the same generation canvas; generative editing can alter non-mouth details."),
        "co_resident_load": "Existing OmniVoice and LTX processes left resident; exact initial whole-device use is in gpu_snapshot.",
    }
