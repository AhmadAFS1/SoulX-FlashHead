"""CPU landmark and mouth-crop diagnostics for source-detail experiment."""
import json
...
def main():
    cv2.setNumThreads(1)
    sheet = Image.new("RGB", (7 * 192, 4 * 150), "#eeeeee")
    draw = ImageDraw.Draw(sheet)
    rows = []
    series = {}
    for row, detail in enumerate(DETAILS):
        path = ROOT / f"detail-{detail}" / "video.mp4"
        ...
        with mp.solutions.face_mesh.FaceMesh(static_image_mode=False, max_num_faces=1,
            ...
            while True:
                ...
                edge_energy.append(float(cv2.Laplacian(gray, cv2.CV_64F).var()))
                timestamp = index / fps
                for column, target in enumerate(TIMES):
                    if column not in samples and abs(timestamp - target) <= .5 / fps + 1e-6:
                        ...
                        draw.text((column * 192 + 4, row * 150 + 3),
                                  f"source {detail}px", fill="black")
                        draw.text((column * 192 + 4, row * 150 + 20),
                                  f"audio {timestamp:.2f}s", fill="black")
                index += 1
        capture.release()
        series[detail] = openings
        rows.append({
            "effective_source_detail_px": detail, "decoded_frames": index,
    ...
    for item in rows:
        detail = item["effective_source_detail_px"]
        item["opening_trajectory_correlation_vs_307"] = correlation(series[307], series[detail])
    sheet.save(ROOT / "mouth-comparison.png")
    (ROOT / "diagnostics.json").write_text(json.dumps({
        "execution": "CPU MediaPipe/XNNPACK analysis of retained GPU-generated videos; no new SoulX inference.",
        "metric_limitations": "Laplacian variance covers the whole mouth crop and includes lips, facial hair, pose and compression; it is not a validated tooth-quality score.",
        "rows": rows}, indent=2) + "\n")
