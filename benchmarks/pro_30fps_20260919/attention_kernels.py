...
def main():
    ...
    result = {
        "status": "starting",
        "date_utc": utc_now(),
        "execution": "fresh GPU kernel-only diagnostic on real activations",
        "environment": environment_manifest(),
        "capture_sha256": sha256(args.captures),
        "cases": [],
    }
    result["source_sha256"] = snapshot_sources(
        output, [Path(__file__), ROOT / "soulx_rtc/pro_attention_backends.py"]
    )
    lease = None
    try:
        ...
        with torch.inference_mode():
            for index, (key, case) in enumerate(sorted(cases.items())):
                ...
                for role in ("q", "k", "v"):
                    row = case[role]
                    path = args.captures.parent / row["path"]
                    if sha256(path) != row["sha256"]:
                        raise ValueError("Attention input hash mismatch")
                    values.append(
                ...
                expected, rt = observations["reference"]
                actual, ct = observations["candidate"]
                result["cases"].append(
                    {
                        "block": key[0],
                        "invocation": key[1],
                        ...
                        "reference": rt,
                        "candidate": ct,
                        "speedup": rt["median_ms"] / ct["median_ms"],
        ...
        result.update(
            status="complete",
            reference_backend=reference.manifest(),
            candidate_backend=candidate.manifest(),
        )
    except Exception as error:
        result["status"] = "failed"
        result["failure"] = failure_record("attention_kernel", error)
        raise
    finally:
        atomic_write_json(output / "results.json", result)
