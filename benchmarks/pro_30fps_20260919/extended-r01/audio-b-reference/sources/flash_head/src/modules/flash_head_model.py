...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs, grid_sizes, rotary=None):
        ...
        if self.use_usp:
            ...
            if self._pro_attention_backend is None:
                x = flash_attention(q=q, k=k, v=v, num_heads=self.num_heads)
            else:
                x = self._pro_attention_backend(q, k, v, self.num_heads, owner=self)
        return self.o(x)
...
class CrossAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, eps: float = 1e-6, has_image_input: bool = False):
        super().__init__()
        self._pro_cross_attention = None
        self.dim = dim
        self.num_heads = num_heads
        self.head_dim = dim // num_heads
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, prepared_kv=None):
        ...
        if prepared_kv is None:
            ...
            k, v = prepared_kv
        attention = self._pro_cross_attention or flash_attention
