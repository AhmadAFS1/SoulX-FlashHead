...
def require_distinct_audio(named_samples):
    """Source file hashes cannot establish that played segments are different."""
    hashes = {}
    for name, samples in named_samples.items():
        digest = hashlib.sha256(samples.tobytes()).hexdigest()
        if digest in hashes.values():
            raise ValueError(f"Duplicate effective speech segment: {name}")
        hashes[name] = digest
    return hashes
...
def main():
    ...
    fixtures = root / "fixtures-correction.json"
    policies = {
        "reference": ROOT
    ...
    tests = [
        ...
        ("sichuan-30s", "indian150-sichuan-30s", 1, 250),
    ]
    from benchmarks.pro_quantization_v2_20260918.run import _audio_slices

    speech = {
        name: _audio_slices(resolve_fixture(fixtures, fixture), 250)[0]
        for name, fixture in [
            ("standard", "indian150-a"),
            ("audio-b", "indian150-audio-b"),
            ("sichuan-30s", "indian150-sichuan-30s"),
        ]
    }
    speech_hashes = require_distinct_audio(speech)
    result = {
        "status": "scheduled",
        "date_utc": utc_now(),
        ...
        "distinct_effective_speech_sha256": speech_hashes,
