...
def int8_convolutions(layers):
    ...
    convs = [item for item in records if "convolution" in str(item.get("LayerType", "")).lower()
             or (item.get('LayerType') == 'correlation' and 'Conv' in item.get('Metadata', ''))]
    quantized = [item for item in convs if item.get("Inputs") and all(
        ...
        and (str(item.get('Weights', {}).get('Type', '')).lower() == 'int8'
...
def export_stage(stage, values, path, precision, scales, floating_precision='bf16'):
    import onnx
    import numpy as np
    ...
    for node in model.graph.node:
        if node.op_type == 'Clip' and len(node.input) >= 2 and node.input[1] and (len(node.input) < 3 or not node.input[2]):
            node.op_type = 'Max'
            del node.input[2:]
    converted = []
    if precision == 'int8':
    ...
    if floating_precision == 'fp16':
        ...
        def convert_tensor(tensor):
            if tensor.data_type == TensorProto.BFLOAT16:
                if tensor.raw_data:
                    array = (np.frombuffer(tensor.raw_data, np.uint16).astype(np.uint32) << 16).view(np.float32)
                else:
                    array = (np.asarray(tensor.int32_data, dtype=np.uint32) << 16).view(np.float32)
        ...
        for node in model.graph.node:
            for attribute in node.attribute:
                ...
                if node.op_type == 'Cast' and attribute.name == 'to' and attribute.i == TensorProto.BFLOAT16:
    ...
    return {'inputs':[{'name':n,'shape':list(v.shape)} for n,v in zip(inputs,values)],
            ...
            'floating_precision':floating_precision, 'caller_cache_dtype':'bfloat16'}
...
def build(args, output, result):
    ...
    result.update(schema_version=1,groups=source['groups'],precision=args.precision,stages={},
                  floating_precision='fp16' if args.precision == 'fp16' else args.floating_precision,
    ...
    for indices in source['groups']:
        ...
        for i,(signature,sample) in enumerate(source['samples'][key].items()):
            ...
            if not all(row['finite'] for row in record['build_sample_comparison']):
                raise RuntimeError('Built stage produced nonfinite output/cache on its calibration input')
