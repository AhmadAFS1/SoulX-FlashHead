...
class TensorRTStage:
    ...
    def __call__(self, *values):
        ...
        for value, spec in zip(values, self.inputs):
            ...
            prepared.append(value.to(self.binding_dtype).contiguous())
        outputs = [torch.empty(spec["shape"], device=self.workspace.device, dtype=self.binding_dtype) for spec in self.outputs]
...
def install_stage_plan(vae, path):
    ...
    def factory(indices, stage):
        ...
        for signature, record in records.items():
            ...
            if floating not in ('bf16', 'fp16') or record.get('floating_precision', 'bf16') != floating:
                raise ValueError('Stage floating precision mismatch')
            if record.get('binding_dtype', 'bfloat16') != ('float16' if floating == 'fp16' else 'bfloat16'):
