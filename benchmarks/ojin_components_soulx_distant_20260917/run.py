"""Postprocess the exact SoulX batch-five receiver recording with disclosed components."""
...
SOURCE=Path('benchmarks/concurrency_15fps_20260916/bf16-batch5-c5-recorded-peer0.mp4')
TASK=Path('models/ojin-components/face_landmarker.task')
ENGINE=Path('models/ojin-components/native2x-srvgg-320x576-trt10.16-cu13.engine')
OLD_WEIGHTS=Path('models/mouth-sr/realesr-general-x4v3.pth')
TIMES=(2.5,4.,6.,8.,10.,11.5)
FPS=15
WIDTH,HEIGHT=320,576
OUTPUT_WIDTH,OUTPUT_HEIGHT=640,1152
...
class Recorder:
    def __init__(self,path):
        ...
        self.stream.width,self.stream.height=OUTPUT_WIDTH,OUTPUT_HEIGHT
...
def main():
    ...
    result=dict(status='starting',date_utc=datetime.now(timezone.utc).isoformat(),
        execution='Postprocessing of the exact existing SoulXFlashHead WebRTC receiver recording; no LTX inference',
    ...
    with acquire_gpu_lease():
        ...
        for mode in ('bicubic2','legacy-general4x-resized2x','FaceLandmarker-native2x-TRT'):
            name=f'SoulX-LITE-default-closeup-character-{mode}-320x576-to-640x1152'
            folder=ROOT/name;folder.mkdir(exist_ok=False)
            recorder=Recorder(folder/(name+'.mp4'))
            tracker=FaceLandmarkerTracker(TASK,fps=FPS) if mode.startswith('Face') else None
            ...
            with Monitor() as monitor:
                ...
                for index,rgb in enumerate(frames()):
                    ...
                    if mode=='bicubic2':out=cv2.resize(rgb,(OUTPUT_WIDTH,OUTPUT_HEIGHT),interpolation=cv2.INTER_CUBIC)
                    ...
                    for timestamp in TIMES:
                        if index==round(timestamp*FPS):Image.fromarray(out).save(folder/f'frame-{timestamp:.1f}s.png')
