"""Write evidence-backed run summary; inference hardware from run manifest."""
import csv
import json
...
def main():
    generation = json.loads((ROOT / 'results.json').read_text())
    diagnostics = json.loads((ROOT / 'diagnostics.json').read_text())
    validation = json.loads((ROOT / 'validation.json').read_text())
    assert generation['status'] == 'complete'
    assert diagnostics['generation_status'] == 'complete'
    assert len(diagnostics['rows']) == len(generation['runs']) == 38
    rows = diagnostics['rows']
    summary = []
    for family, values in [('shift', generation['shifts']), ('history', generation['histories'])]:
        for value in values:
            selected = [r for r in rows if r['seed'] in generation['repeat_seeds']
                        and r[family] == value
                        and (r['history'] == 2 if family == 'shift' else r['shift'] == 5)]
            assert len(selected) == 3
            item = dict(parameter=family, value=value,
                        detected_frames=sum(r['detected_frames'] for r in selected), total_frames=750)
            for metric in ['mouth_opening_mean', 'mouth_opening_p95', 'roll_span_degrees']:
                ratios = [r['relative_to_same_seed_baseline'][metric] for r in selected if r['metrics']]
                item[metric + '_median_change_pct'] = float((np.median(ratios) - 1) * 100)
                item[metric + '_seeds_decreased'] = sum(x < 1 for x in ratios)
            item['opening_correlation_median'] = float(np.median([r['opening_correlation_vs_baseline'] for r in selected]))
            summary.append(item)
    (ROOT / 'summary.json').write_text(json.dumps(dict(
        execution=generation['execution'], gpu_snapshot=generation['gpu_snapshot'],
        date_utc=generation['date_utc'], summary=summary,
        caveat='Medians of per-seed ratios across three seeds on one portrait/utterance. Motion proxies are not naturalness, teeth or lip-sync quality scores.'), indent=2) + '\n')
    with (ROOT / 'metrics.csv').open('w', newline='') as handle:
        columns = ['name', 'seed', 'shift', 'history', 'detected_frames', 'mouth_opening_mean',
                   'mouth_opening_p95', 'roll_span_degrees', 'horizontal_span_eye_units',
                   'vertical_span_eye_units', 'opening_correlation_vs_baseline']
        writer = csv.DictWriter(handle, fieldnames=columns)
        writer.writeheader()
        for row in rows:
            merged = dict(row, **row['metrics'])
            writer.writerow({key: merged.get(key) for key in columns})
    text = '''# Wide movement-parameter experiments
...
'''
    for row in summary:
        text += (f"| {row['parameter']} | {row['value']:g} | {row['mouth_opening_mean_median_change_pct']:+.1f}% | "
                 f"{row['mouth_opening_p95_median_change_pct']:+.1f}% | {row['roll_span_degrees_median_change_pct']:+.1f}% | "
                 f"{row['roll_span_degrees_seeds_decreased']}/3 | {row['opening_correlation_median']:.3f} | {row['detected_frames']}/750 |\n")
    text += '''
